...
public class DBEnquiry {
	...
	public static ZabbixItem[] execute(Query[] _queries, DBConnManager _dbcm,
			String dbname) {
		...
		try {
			_conn = _dbcm.getDs().getConnection();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			SmartLogger.logThis(Level.ERROR, "ERROR on DBEnquiry while getting connection for "+dbname+" "+e);
		}
		...
		for (int i = 0; i < _queries.length; i++) {
			...
			try {
				...
				if (_queries[i].getActive()) {
					...
					if (now.after(nextRun)) {
						...
						if (racecond) {
							...
							ZabbixItem zitem = new ZabbixItem(Constants.ITEM_PREFIX+_dbcm.getDBType().getDBTypeString()+"."
							+_queries[i].getName(), result,dbname);
							SZItems.add(zitem);
				...
				ZabbixItem zitem = new ZabbixItem(Constants.ITEM_PREFIX+_dbcm.getDBType().getDBTypeString()+"."
											+_queries[i].getName(),	tempStr , dbname);
				SZItems.add(zitem);
