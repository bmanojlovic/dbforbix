...
import com.smartmarmot.dbforbix.db.DBConn;
	...
	public static Integer  getSleep() throws Exception {
		try{
			...
			try{
				...
				logThis(Level.WARN,"Warning while getting "+Constants.DBFORBIX_DAEMON_SLEEP+" I will use the default "+sleep);
			}
			return sleep;

		} catch (Exception ex){

			logThis(Level.ERROR,"Error on Configurator while retriving "+Constants.DBFORBIX_DAEMON_SLEEP+" "+ex);
			return null;
		}
	}
